<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="space-y-6 border-gray-100 p-5 sm:p-6 dark:border-gray-800">
        <form action="{{ route('kiosk.employee.update', $employee->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="-mx-2.5 flex flex-wrap gap-y-5">
                ...
                <div class="w-full px-2.5 xl:w-1/2">
                    ...
                    <input type="text" name="id_number" id="id_number"
                        value="{{ old('id_number', $employee->id_number) }}" placeholder="Masukan nomor id"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                    @error('id_number')
                    <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="w-full px-2.5 xl:w-1/2">
                    ...
                    <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                        <select name="shift" id="shift" value="{{ old('name', $employee->name) }}"
                            ...
                            <option value="pagi" {{ old('shift', $employee->shift) === 'pagi' ? 'selected' : '' }}>
                                Pagi
                            </option>

                            <option value="malam" {{ old('shift', $employee->shift) === 'malam' ? 'selected' : '' }}>
                                Malam
                            </option>
                        </select>
                        <span
                ...
                <div class="w-full px-2.5 xl:w-1/2">
                    ...
                    <input type="text" name="contact" id="contact" value="{{ old('contact', $employee->contact) }}"
