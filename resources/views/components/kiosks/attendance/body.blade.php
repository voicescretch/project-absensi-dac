        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                ...
                <td class="px-6 py-3 flex items-center justify-end">
                    <button id="task-{{ $attendance->id }}-dropdown-button"
                        data-dropdown-toggle="task-{{ $attendance->id }}-dropdown"
                    ...
                    <div id="task-{{ $attendance->id }}-dropdown"
                        class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                        <ul class="py-1 text-sm" aria-labelledby="task-{{ $attendance->id }}-dropdown-button">
                            <li>
                                <button data-modal-target="deleteAttendanceModal-{{ $attendance->id }}"
                                    data-modal-toggle="deleteAttendanceModal-{{ $attendance->id }}"
            ...
            <!-- Delete Modal -->
            <div id="deleteAttendanceModal-{{ $attendance->id }}"
                class="fixed inset-0 hidden items-center justify-center p-5 overflow-y-auto modal z-999999">
                <div data-modal-target="deleteAttendanceModal-{{ $attendance->id }}"
                    data-modal-toggle="deleteAttendanceModal-{{ $attendance->id }}"
                ...
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                            Apakah Anda yakin ingin menghapus data presensi yang satu ini?
                        </p>

                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteAttendanceModal-{{ $attendance->id }}"
                                data-modal-toggle="deleteAttendanceModal-{{ $attendance->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="{{ route('kiosk.attendance.destroy', $attendance->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
